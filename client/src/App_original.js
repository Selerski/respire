...
function App() {
  ...
  const updateAddress = newAddress => {
    setAddresses(addresses =>
      addresses.map((addr, k) =>
        newAddress._id === addr._id ? newAddress : addr
      )
    );
  };
    ...
    <div className="App">
      <div className="Header">
        ...
        <select
          className="Header"
          value={showBlocked}
          onChange={toggleShowBlock}
        >
          <option value="false"> Browsing History </option>
      ...
      <div className="Lists Containers">
        {!showBlocked ? (
          <NotBlockedList
            addressList={addresses.filter(
              addr => addr.blockedStatus === 'notBlocked'
            )}
            status="notBlocked"
            functions={functions}
          />
      ...
      {locked ? (
        <div>
          <p className="ListHeader">
            {' '}
            YOU CANT UNBLOCK FOR {secondsToHms(timeRemaining)}{' '}
          </p>
        ...
        <div>
          <p className="ListHeader">
            {' '}
            Those are the domains you blocked: BE STRONG
          </p>
          <button className="LockForm" onClick={lock}>
            {' '}
            Lock Yourself Out :)
          </button>
          <select
            className="LockForm"
            value={durationInSec / 1000}
...
function NotBlockedList({ addressList, status, functions }) {
  return (
    <div className={`BlockedList`}>
      <p className="ListHeader2">
        {' '}
        Domains you visited :) Are you sure they help you be happy? :) :) :)
      </p>
      {addressList.map(addr => (
        <div>
      ...
      {hover ? (
        <div>
          ...
          <button
            ...
            onClick={() => functions.timeblock(address._id, durationInSec)}
          >{`BLOCK`}</button>
          <div>
            {show ? (
              <div>
                ...
                {address.subdomains.map(subd =>
                  subd.name ? (
                    <p className="Subdomain">{subd.name}</p>
                  ) : (
                    <p>www.</p>
                  )
                )}
...
function BlockedAddress({ address, functions, locked }) {
  ...
  const [timeRemaining, setTimeRemaining] = useState(
    Date.parse(address.blockedDate) - Date.now() + address.blockedTimePeriod
  );
  function toggleDetails() {
    setShow(show => !show);
  }
    ...
    <div className="Address">
      ...
      {show ? (
        <div>
          ...
          {address.subdomains.map(subd =>
            subd.name ? <p>{subd.name}</p> : <p>www.</p>
          )}
          {
            <button className="Details" onClick={toggleDetails}>
              {' '}
      ...
      {locked ? (
        <button
          className="UnblockImpossible"
          onClick={() => alert('CHEAAAATERR 🦊')}
        >
          UNBLOCK (YOU WISH) 🙉🙈🙊
        </button>
      ) : (
        <button
          className="Details"
          onClick={() => functions.unblock(address._id)}
        >
          UNBLOCK 😞
        </button>
      )}
